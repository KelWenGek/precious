	...
	StickyTab.prototype = {
		...
		_simulateSticky: function () {
			...
			var c = this,
				...
				o = c.withinParent,
				p = c._originCssText,
				q = j.top - g + Math.abs(window.lib.Animation.getTransformOffset($('#J_detail_main')[0]).y),
			...
			//t = p + convertToString({
			//		position: "absolute",
			//		top     : Math.abs(window.lib.Animation.getTransformOffset($('#J_detail_main')[0]).y) + "px",
			//		width   : k.width
			//	}),
				u = 1,
			...
			//tab条固定位置的3种方式
				v = c._scrollHandler = setHandlerTimeout(function () {
						...
						var b = Math.abs(window.lib.Animation.getTransformOffset($('#J_detail_main')[0]).y),
							s = p + convertToString({
									position    : "absolute",
									"z-index"   : '100000000000',
									left        : '0',
			...
			//$('#J_detail_main')[0].parentNode.addEventListener('pan', v,!1);
