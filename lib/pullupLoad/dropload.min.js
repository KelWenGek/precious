...
!function (a) {
    ...
    function i(b, c) {
        ...
        c._moveY > 0
            ? c.direction = "down"
            : c._moveY < 0 && (c.direction = "up");
        var d = Math.abs(c._moveY);
        if ("" != c.opts.loadUpFn && c.touchScrollTop <= 0 && "down" == c.direction && !c.isLockUp) {
            b.preventDefault();
            c.$domUp = a("." + c.opts.domUp.domClass);
            if (!c.upInsertDOM) {
                c.$element.prepend('<div class="' + c.opts.domUp.domClass + '"></div>');
                c.upInsertDOM = !0;
            }
            n(c.$domUp, 0);
            if (d <= c.opts.distance) {
                c._offsetY = d;
                c.$domUp.html(c.opts.domUp.domRefresh);
            } else if (d > c.opts.distance && d <= 2 * c.opts.distance) {
                c._offsetY = c.opts.distance + .5 * (d - c.opts.distance);
                c.$domUp.html(c.opts.domUp.domUpdate);
            } else {
                c._offsetY = c.opts.distance + .5 * c.opts.distance + .2 * (d - 2 * c.opts.distance);
            }
            c.$domUp.css({height: c._offsetY});
        }
    }

    function j(b) {
        var c = Math.abs(b._moveY);
        if ("" != b.opts.loadUpFn && b.touchScrollTop <= 0 && "down" == b.direction && !b.isLockUp) {


            n(b.$domUp, 300);
            if (c > b.opts.distance) {
                b.$domUp.css({
                    height: b.$domUp.children()
                             .height()
                });
    ...
    function l(a) {
        a._scrollContentHeight = a.opts.scrollArea == b
            ? e.height()
            : a.$element[0].scrollHeight;
    }
    ...
    f.prototype.init = function (f) {
        ...
        if ("" != l.opts.loadDownFn) {
            l.$element.append('<div class="' + l.opts.domDown.domClass + '">' + l.opts.domDown.domRefresh + "</div>");
            l.$domDown = a("." + l.opts.domDown.domClass)

        }


        l._threshold = l.$domDown && "" === l.opts.threshold
            ? Math.floor(1 * l.$domDown.height() / 3)
            : l.opts.threshold;

        if (l.opts.scrollArea == b) {

            l.$scrollArea = d;
            l._scrollContentHeight = e.height();
            l._scrollWindowHeight = c.documentElement.clientHeight;

        } else {

            l.$scrollArea = l.opts.scrollArea;
        ...
        d.on("resize", function () {
            l._scrollWindowHeight = l.opts.scrollArea == b
                ? b.innerHeight
                : l.$element.height()
        });
    ...
    f.prototype.lock = function (a) {
        ...
        void 0 === a
            ...
            : "down" == b.direction
            ? b.isLockUp = !0
            : (b.isLockUp = !0, b.isLockDown = !0)
            : "up" == a
            ? b.isLockUp = !0
            : "down" == a && (b.isLockDown = !0, b.direction = "up");
    };
    ...
    f.prototype.resetload = function () {
        var b = this;
        if ("down" == b.direction && b.upInsertDOM) {
        ...
        } else if ("up" == b.direction) {
            b.loading = !1;
            if (b.isData) {

                b.$domDown.html(b.opts.domDown.domRefresh);
                l(b);
                k(b);
            } else {
