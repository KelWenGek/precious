;(function (a, c) {
        ...
        constructor: b, render: function () {
            ...
            this.options.dataSource.data(f.data(), function (g) {
                ...
                a.each(g.data, function (h, j) {
                    var i;
                    if (j.type === "folder") {
                        i = e.$element.find(".tree-folder:eq(0)")
                             .clone()
                             .show();
                        i.find(".tree-folder-name")
                         .html(j.name);
                        i.find(".tree-loader")
                         .html(e.options.loadingHTML);
                        var k = i.find(".tree-folder-header");
                        k.data(j);
                        if ("icon-class" in j) {
                            k.find('[class*="icon-"]')
                             .addClass(j["icon-class"])
                        }
                    } else {
                        if (j.type === "item") {
                            i = e.$element.find(".tree-item:eq(0)")
                                 .clone()
                                 .show();
                            i.find(".tree-item-name")
                             .html(j.name);
                            i.data(j);
                            if ("additionalParameters" in j && "item-selected" in j.additionalParameters && j.additionalParameters["item-selected"] == true) {
                                ...
                                i.find("i")
                                 .removeClass(e.options["unselected-icon"])
                                 .addClass(e.options["selected-icon"])
                            }
                        }
                    }
                    if (f.hasClass("tree-folder-header")) {
                        f.parent()
                         .find(".tree-folder-content:eq(0)")
                         .append(i)
                    } else {
                        f.append(i)
                    }
            ...
            if (this.options.multiSelect) {
                ...
                if (g[0] !== d[0]) {
                    g.removeClass("tree-selected")
                     .find("i")
                     .removeClass(this.options["selected-icon"])
            ...
            if (d.hasClass("tree-selected")) {
                ...
                d.find("i")
                 .removeClass(this.options["unselected-icon"])
            ...
            if (d.find("." + this.options["close-icon"]).length) {
                if (f.find(".tree-folder-content")
                     .children().length) {
                    f.find(".tree-folder-content:eq(0)")
                     .show()
                } else {
                    this.populate(d)
                }
                ...
                f.find("." + this.options["open-icon"] + ":eq(0)")
    ...
    a.fn.tree = function (d) {
        ...
        this.each(function () {
            ...
            html += '<i class="' + c["close-icon"] + '"></i>';
            html += '<div class="tree-folder-name"></div> </div>';
            html += '<div class="tree-folder-content"></div>';
            html += '<div class="tree-loader" style="display:none"></div></div>';
            html += '<div class="tree-item" style="display:none;"> ';
            html += c["unselected-icon"] == null ? "" : '<i class="' + c["unselected-icon"] + '"></i>';
            html += '<div class="tree-item-name"></div></div>';
